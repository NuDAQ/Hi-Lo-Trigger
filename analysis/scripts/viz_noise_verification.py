...
def causal_rolling_or(arr, window_size):
    """
    O(1) sliding window equivalent to VHDL shift-register OR logic.
    Eliminates array slicing bottlenecks via cumulative sum differentiation.
    """
    c_sum = np.cumsum(arr.astype(np.int8), axis=-1, dtype=np.int16)
    out = np.zeros_like(arr, dtype=bool)
    
    out[..., :window_size] = c_sum[..., :window_size] > 0
    out[..., window_size:] = (c_sum[..., window_size:] - c_sum[..., :-window_size]) > 0
    return out
...
def generate_thermal_verification():
    ...
    output_pdf = os.path.abspath(os.path.join(script_dir, "../thermal_verification.pdf"))
    ...
    for snr_str, data in vhdl_results.items():
        ...
        if events > 0:
            ...
            hw_far_vals.append(max(far, 1e-8)) # Log-scale minimum floor
            print(f"    VHDL SNR {snr}: {fp} FPs over {events} events (FAR: {far:.4e})")

    # 4. Continuous Python Emulation (Bit-Accurate + O(1) Architecture)
    total_py_events = len(chunk_files) * 25000
    
    # Sweep from SNR 1.5 to 5.0 in 0.1 increments
    snr_sweep = np.round(np.arange(1.5, 5.1, 0.1), 1)
    ...
    # 6. ROOT Plotting
    print(f"\n[*] Generating ROOT plots at {output_root}...")
    ...
    mg_rate.SetTitle("Hi-Lo Trigger FAR: Quantized Emulation vs. VHDL;Threshold (SNR);False Alarm Rate (Triggers / Event)")
    ...
    mg_rate.Draw("A")
    mg_rate.SetMinimum(1e-8)
    
    leg_rate = ROOT.TLegend(0.55, 0.65, 0.88, 0.85)
    leg_rate.SetBorderSize(1)
    leg_rate.AddEntry(g_py_rate, "Quantized Emulation (Line)", "l")
    ...
    root_file.Close()
    print(f"[*] Execution complete. Outputs verified and logged.")
